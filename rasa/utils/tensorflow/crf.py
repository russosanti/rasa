...
def crf_decode(potentials, transition_params, sequence_length):
    ...
    def _multi_seq_fn():
        ...
        # potentials output of transformer (batch-size x seq-len x num-tags)
        ...
        # backpointers = last state of RNN (batch-size x seq-len - 1 x tag-num)
        # last-score: most likely tag at last position of seq (batch-size x tag-num)
        ...
        # backward path of "viterbi alogrithm" in TF
        # propagatet the most likely last tag back to start of sequence to get
        ...
        # decode_tags: most likely tags for sequence (batch-size x seq-len)

        best_score = tf.reduce_max(last_score, axis=1)

        # best_score: best score in seq (batch-size)
